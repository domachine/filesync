...
** Daemon functionality
   ...
   #+BEGIN_SRC: bash
# This sets the depth option. (optional)
DEPTH=-1

# Specifies the source directory for the synchronization.
SOURCE_DIR=

# Specifies the target directory for the synchronization.
TARGET_DIR=

# Specifies the exclude regex. (optional)
EXCLUDE=

# Specifies the path to the filesync executable (optional)
FILESYNC_EXE=filesync

# Specifies the directory to place to put the PID files in.
# !: This directory should only contain the PID files.
# (optional)
RUN_DIR=/tmp/filesync

# This can be used to pass additional params to filesync.
# (optional)
ADDITIONAL_PARAMS=()

# Specifies the log-file. (default stderr). (optional)
LOG_FILE=
   #+END_SRC
...
* Plans
** TODO Design filesync as daemon that called for work
...
*** TODO Interface to communicate
    Sockets will be used as communication interface.
